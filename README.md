# 🐳 Docker Compose: WordPress 5.9 + MySQL + Caddy
...
|-----------|---------|--------------------------------------------------|
| WordPress | 5.9.3   | 📝 Legacy release for compatibility testing      |
...
```bash
GID=$(id -g) docker compose -p wp59 --env-file .env.wp-5.9 up -d
```
...
4. 🏁 Start the stack:
   ...
   > docker compose --env-file .env.wp-5.9 config
   >
   > # Without --env-file, you'll see warnings about unset variables
   > docker compose config
   > ```

5. 🎉 Visit `https://wptesterx59.com:8443` and complete the WordPress installation wizard.
...
├── .env.wp-5.9         # 🔑 Actual env file for this branch (gitignored)
├── Caddyfile           # 🌐 Caddy reverse proxy configuration
├── docker-compose.yml  # 🐳 Docker Compose service definitions
├── wordpress-59/       # 📝 WordPress files (created on first run)
├── db_data-59/         # 🗄️ MySQL data (created on first run)
└── README.md
```
...
|------------------|-------------|--------------------------------------|
| `./wordpress-59` | Local folder | 📝 WordPress files (`/var/www/html`) |
| `./db_data-59`   | Local folder | 🗄️ MySQL data (`/var/lib/mysql`)     |
...
# 📋 View logs
docker compose -p wp59 logs -f

# ⏹️ Stop the stack
docker compose -p wp59 down

# 💥 Stop and remove volumes (destroys all data)
docker compose -p wp59 down -v

# 📂 Connect via SFTP
sftp -P 2223 wpadmin@localhost
```
...
docker compose -p wp59 cp caddy:/data/caddy/pki/authorities/local/root.crt ./caddy_local_ca.crt
